    ...
    for i in range(sprite.shape[0]):
        for j in range(sprite.shape[1]):
            if background is None or sprite[i, j] != background:
                ...
                if 0 <= blit_x + i < grid.shape[0] and 0 <= blit_y + j < grid.shape[1]:
                    new_grid[blit_x + i, blit_y + j] = sprite[i, j]
...
def blit_sprite(grid, sprite, x, y, background=Color.BLACK):
    ...
    """
    blit_sprite_x, blit_sprite_y = int(x), int(y)

    for i in range(sprite.shape[0]):
        for j in range(sprite.shape[1]):
            if background is None or sprite[i, j] != background:
                if 0 <= blit_sprite_x + i < grid.shape[0] and 0 <= blit_sprite_y + j < grid.shape[1]:
                    grid[blit_sprite_x + i, blit_sprite_y + j] = sprite[i, j]
...
def crop(grid, background=Color.BLACK):
    ...
    for _x in range(n):
        for _y in range(m):
            if grid[_x, _y] != background:
                x_min = min(x_min, _x)
                x_max = max(x_max, _x)
                y_min = min(y_min, _y)
                y_max = max(y_max, _y)

    _x, _y, _w, _h = x_min, y_min, x_max - x_min + 1, y_max - y_min + 1
    return grid[_x : _x + _w, _y : _y + _h]
